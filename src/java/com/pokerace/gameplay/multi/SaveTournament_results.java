...
public class SaveTournament_results {
	...
	void save_results() {
		...
		for (Player ply : players) {
			...
			if (ply.getM_Name().length() > 1) {
				...
				creditpoint += (int) ply.getM_Credit() + ",";
	...
	public void save_recent_winners() {
		...
		CouchDbClient rankingDb = CouchDBUtil.getClient(CouchDatabase.TOURNAMENT_RANKING); // new CouchDbClient(dbProperties4);
		CouchDbClient usersDb = CouchDBUtil.getClient(CouchDatabase.USERS); // new CouchDbClient(dbProperties5);
		CouchDbClient botsDb = CouchDBUtil.getClient(CouchDatabase.BOTS); // new CouchDbClient(dbProperties6);
		...
		for (int j = 0; j < 3; j++) {
			...
			List<JsonObject> list = rankingDb.view("_all_docs").query(JsonObject.class);
			String nickname = "";
			String player_level = "";
			try {
